...
object printv {
  ...
  def apply[T<:Data:ru.TypeTag](sig:T, module:String, fmtmap:Map[String,String]=Map[String,String]()) = {
...
object dumps {
  ...
  implicit class Decoupled_EXU_LSMDU_IO_Dump(data:DecoupledIO[EXU_LSMDU_IO]) {
    def dump(msg:String) = {
      printf("%d: "+msg+": [%b,%b]: wb={v:%b, id:%x, pc:%x, instr:%x, rd:%d, wen:%b, data:%x}, ops={fu_type=%d, fu_op=%d, op1=%x, op2=%x}, is_cached=%b\n", GTimer(), data.valid, data.ready, data.bits.wb.v, data.bits.wb.id, data.bits.wb.pc, data.bits.wb.instr.asUInt, data.bits.wb.rd_idx, data.bits.wb.wen, data.bits.wb.data, data.bits.ops.fu_type, data.bits.ops.fu_op, data.bits.ops.op1, data.bits.ops.op2, data.bits.is_cached)
    }
  }
