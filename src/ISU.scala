...
class ISU extends Module {
  ...
  // bypass
  val bypass_valids = Seq(io.alu_bypass.valid, io.mdu_bypass.valid, io.lsu_bypass.valid, io.bru_bypass.valid)
  ...
  val fu_ready = io.alu.ready && io.mdu.ready && io.bru.ready && io.lsu.ready
  val outfire = io.alu.fire() || io.mdu.fire() || io.bru.fire() || io.lsu.fire()
